...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        setup_db(self.app, f'{SQLALCHEMY_DATABASE_URI}_test')
    ...
    # TODO [X] POST RAND /api/v1.0/questions/random with previous should return 200
    def test_get_random_question_with_previous(self):
        ...
        data = json.loads(res.data)
        self.assertEqual(res.status_code, status.HTTP_200_OK )
        self.assertEqual(data['available'], len(question_list) - (len(_json['previous']) +1) )
        self.assertIsInstance(data['data'], dict )
        self.assertIsNone(data['category'])
        self.assertEqual(len(data['previous']), 3)    
        self.assertIn(data['data']['id'], data['previous'])    
        
    # TODO [X] POST RAND /api/v1.0/questions/random with bad category should return 422
    ...
    # TODO [X] POST RAND /api/v1.0/questions/random with invalid previous ids should return 200
    def test_get_random_question_with_bad_previous(self):
        ...
        self.assertIsInstance(data['data'], dict )
        self.assertIsNone(data['category'])
        # The api disregards bad previous ids so the available should return the actual count
    ...
    def test_delete_question(self):
        """Test should return 200 """
        question = Question(question = 'question 666',answer = 'answer 666', category = 1, difficulty = 5 )
        id = None
        with self.app.app_context():
            self.db = SQLAlchemy()
            self.db.init_app(self.app)
    ...
    # TODO [X] POST FIND /api/v1.0/questions/search should return 422 if search not valid
